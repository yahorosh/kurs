 ...
 * @author horoshev
 */
class Controller_Rest_Dean extends Controller_Rest {
    ...
    function action_get() {
        ...
        if ($itemId = $this->request->params('id')) {
            $items = $em->getRepository('Entity\Dean')->find($itemId);
            $items = $items ? $items->as_array() : null;
        } else {
            $q = $em->createQuery('SELECT f FROM Entity\Dean f ORDER BY f.name');
    ...
    function action_put() {
        ...
        try {
            ...
            $item = $em->getRepository('Entity\Dean')->find($itemId);
            if (!$item) {
                $code = 404;
