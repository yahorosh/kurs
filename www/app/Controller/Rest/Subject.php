 ...
 * @author horoshev
 */
class Controller_Rest_Subject extends Controller_Rest {
    ...
    function action_get() {
        ...
        if ($itemId = $this->request->params('id')) {
            $items = $em->getRepository('Entity\Subject')->find($itemId);
            $items = $items ? $items->as_array() : null;
        } else {
            $q = $em->createQuery('SELECT sj FROM Entity\Subject sj ORDER BY sj.name');
    ...
    function action_put() {
        ...
        try {
            ...
            $item = $em->getRepository('Entity\Subject')->find($itemId);
            if (!$item) {
                $code = 404;
