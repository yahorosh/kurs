 ...
 * @author horoshev
 */
class Controller_Rest_Faculty extends Controller_Rest {
    ...
    function action_get() {
        ...
        if ($itemId = $this->request->params('id')) {
            $items = $em->getRepository('Entity\Faculty')->find($itemId);
            $items = $items ? $items->as_array() : null;
        } else {
            $q = $em->createQuery('SELECT f FROM Entity\Faculty f ORDER BY f.name');
    ...
    function action_post() {
        ...
        $item = new Entity\Faculty();


        if ($item->from_array($json)) {
            $em->persist($item);
            $em->flush();
            $view->setData($item->to_array('dean'));
        } else {
    ...
    function action_put() {
        ...
        try {
            ...
            $item = $em->getRepository('Entity\Faculty')->find($itemId);
            if (!$item) {
                $code = 404;
